...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            windowWidth = im0.shape[1]
            windowHeight = im0.shape[0]
            ...
            if len(det):
                ...
                bestPerson = None
                revDet = reversed(det)
                for i in range(0, len(revDet)):
                    if names[int(revDet[i, 5])] == "jetbot":
                        if bestPerson is None:
                            bestPerson = revDet[i]
                        elif bestPerson[4] < revDet[i, 4]:
                            bestPerson = revDet[i]
                        break
                    bestPerson = None
                    # if revDet[i]
                ...
                tempControl = str()
                if bestPerson is not None:
                    label = f'{names[int(bestPerson[5])]} {bestPerson[4]:.2f}'
                    left, top, right, bottom = int(bestPerson[0]), int(bestPerson[1]), int(bestPerson[2]), int(
                        bestPerson[3])
                    ...
                    plot_one_box(bestPerson[:4], im0, label=label, color=colors[int(bestPerson[5])], line_thickness=3)
                else:
                    tempControl = f"none|0.0|0.0"

                if jetbotControl != tempControl:
                    jetbotControl = tempControl
                    print(jetbotControl)
                    sock.sendto(jetbotControl.encode(), (ADDRESS, PORT))
